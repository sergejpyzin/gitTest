...
> **.gitkeep** - это пустой файл, который пользователи Git создают, так что в репозитории Git сохраняется пустой каталог проекта. По соглашению, пустые каталоги, не содержащие файлов, не передаются в репозитории Git. Однако один GITKEEP-файл в каталоге заставляет Git сохранять каталог в репозитории. Расширение файла ".gitkeep" не является частью официального стандарта Git, но принято по соглашению с некоторыми Git пользователи;

> **.gitignore** -  файл внутри папки проекта, который игнорирует/предотвращает передачу файлов в локальные и удаленные репозитории;
